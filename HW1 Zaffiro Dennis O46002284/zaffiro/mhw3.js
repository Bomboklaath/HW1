...
function cerca(event){
  ...
  const text=encodeURIComponent(img.value);
...
function rimuoviBoxPreferito(id2){
  ...
  function creaPreferito(boxDaCopiare){
    ...
    big.textContent = boxDaCopiare.querySelector('h1').textContent;
  
    const unlike_button = createImage(x);
...
if(form.email.value.length == 0 ||
    form.nome.value.lenght == 0 )
     {
      alert("Compilare tutti i campi per la corretta iscrizione!");
      event.preventDefault();
    }
